...
interface ReviewCardProps {
  review: GoogleReview;
  cfg: WidgetConfig;
  index?: number; // position within current page — drives stagger delay
}

export function ReviewCard({ review, cfg, index = 0 }: ReviewCardProps) {
  ...
  const cardStyle: React.CSSProperties = {
    ...
    // Entry animation — stagger via delay
    animationDelay: `${index * 60}ms`,
  };

  return (
    <article className="pax-card-enter flex flex-col gap-3 p-5" style={cardStyle}>
      {/* Reviewer row */}
          ...
          <div className="min-w-0 flex-1">
            {el.showName && (
              <p
                ...
                style={{ fontSize: `${cfg.fontSizeTitle}px`, color: cfg.colorText }}
              >
                {review.author_name}
              </p>
            )}
            <div className="flex flex-wrap items-center gap-2 mt-0.5">
              {el.showDate && (
                <span className="text-xs" style={{ color: cfg.colorMuted }}>
      ...
      {el.showReply && review.owner_response && (
        ...
        >
          <p className="text-xs font-semibold mb-1" style={{ color: cfg.colorMuted }}>
            Response from the owner
          </p>
          <p className="text-xs leading-relaxed" style={{ color: cfg.colorText }}>
